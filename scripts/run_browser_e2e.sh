...
"$chrome_bin" --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage   --remote-debugging-port=9222 --user-data-dir=/tmp/ltf-chrome about:blank >/tmp/ltf-e2e-chrome.log 2>&1 &
chrome_pid=$!
cleanup() {
...
for _ in $(seq 1 80); do
  if curl --fail --silent http://127.0.0.1:8000/health >/dev/null     && curl --fail --silent http://127.0.0.1:5173/ >/dev/null     && curl --fail --silent http://127.0.0.1:9222/json/version >/dev/null; then
    break
  fi
  sleep 0.25
done
